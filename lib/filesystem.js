...
FS.prototype.FillDirectory = async function(dirid) {
    ...
    if(this.is_forwarder(inode))
    {
        ...
        await this.follow_fs(inode).FillDirectory(inode.foreign_id);
        return;
    }
    ...
    for(const [name, id] of inode.direntries)
    {
        ...
        const packed = await marshall.MarshallOffthread(
            ["Q", "d", "b", "s"],
            [child.qid,
        ...
        data.set(packed.subarray(offset), offset);
        offset += packed.byteLength - offset;
    }
};
